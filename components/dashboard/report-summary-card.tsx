...
export function ReportSummaryCard({
  ...
  value: string;
  unit: string;
  /** Omit all three (e.g. for a custom date range, which has no natural "previous period") to hide the delta row entirely. */
  deltaPercent?: number;
  up?: boolean;
  compareLabel?: string;
}) {
  return (
    <div className="flex flex-1 flex-col gap-3 rounded-[20px] bg-card p-[22px_24px] shadow-[0px_8px_17.5px_rgba(0,0,0,0.03)]">
      ...
      {deltaPercent !== undefined && up !== undefined && compareLabel && (
