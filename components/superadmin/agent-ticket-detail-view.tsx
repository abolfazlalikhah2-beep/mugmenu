...
import { Calendar, Paperclip, Send, Store } from "lucide-react";
import { TicketMessageBubble } from "@/components/dashboard/ticket-message-bubble";
import { TicketStatusBadge, TICKET_CATEGORY_LABEL } from "@/components/dashboard/ticket-row";
import { PriorityTag } from "@/components/superadmin/inbound-ticket-row";
...
import { addAgentReplyAction, type ActionState } from "@/features/superadmin/routes/actions";
import type { TicketDetail } from "@/features/superadmin/services/ticket-service";

const initialState: ActionState = {};

export function AgentTicketDetailView({ ticket }: { ticket: TicketDetail }) {
  const action = addAgentReplyAction.bind(null, ticket.id);
  ...
  const owner = ticket.business.owners[0];
    ...
    <div className="flex h-full flex-col gap-[14px] md:gap-[18px]">
      <div className="flex flex-wrap items-start justify-between gap-3.5 rounded-[22px] bg-card p-[18px] shadow-[0px_8px_17.5px_rgba(0,0,0,0.03)] md:p-[22px_26px]">
        <div className="flex flex-col gap-2 text-right">
          ...
          <div className="flex items-center gap-1.5 text-[#777]">
            <Store size={14} className="text-[#9A9A9A]" />
      ...
      <div className="flex flex-1 flex-col gap-5 overflow-auto rounded-[22px] bg-card p-4 shadow-[0px_8px_17.5px_rgba(0,0,0,0.03)] md:p-[24px_28px]">
        {ticket.messages.map((m) => (
          <TicketMessageBubble key={m.id} message={m} mine={m.authorType === "AGENT"} />
        ))}
      </div>
