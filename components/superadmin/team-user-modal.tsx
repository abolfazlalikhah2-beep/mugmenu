...
import { PLATFORM_ROLE_LABEL } from "@/components/superadmin/team-user-row";
import type { PlatformRole } from "@/lib/generated/prisma/enums";

const ROLE_OPTIONS: { value: PlatformRole; desc: string }[] = [
  { value: "ADMIN", desc: "دسترسی کامل به همه بخش‌های پنل داخلی" },
  { value: "SUPPORT", desc: "پاسخ‌گویی به تیکت‌ها و مشاهده مشتریان" },
  { value: "FINANCE", desc: "مشاهده تراکنش‌ها، درگاه و گزارش مالی" },
  { value: "VIEWER", desc: "فقط مشاهده، بدون امکان تغییر" },
];

export interface TeamUserFormValue {
  id: string;
  fullName: string;
  phone: string;
...
export function TeamUserModal({ user, onClose }: { user: TeamUserFormValue | null; onClose: () => void }) {
  const action = user ? updateTeamMemberAction.bind(null, user.id) : createTeamMemberAction;
  const [state, formAction, pending] = useActionState(action, initialState);
  const [role, setRole] = useState<PlatformRole>(user?.platformRole ?? "SUPPORT");
  ...
  return (
    <ModalShell
      ...
      subtitle={user ? "اطلاعات و سطح دسترسی این کاربر را ویرایش کنید" : "یک عضو تیم داخلی ماگ‌منو با سطح دسترسی مشخص اضافه کن"}
      onClose={onClose}
      footer={
        <>
          <button
    ...
    >
      <form id="team-user-form" action={formAction} className="flex flex-col gap-[18px]">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          ...
          <Input name="phone" label="شماره تماس" dir="ltr" className="flex-1 text-right" defaultValue={user?.phone} required />
        </div>
        <Input name="platformTeam" label="تیم (اختیاری)" defaultValue={user?.platformTeam ?? ""} placeholder="مثلاً پشتیبانی، مالی…" />
        <div className="flex flex-col gap-3">
          ...
          {ROLE_OPTIONS.map((o) => (
            ...
            >
              <div>
                <div className="text-[15px] font-medium">{PLATFORM_ROLE_LABEL[o.value]}</div>
