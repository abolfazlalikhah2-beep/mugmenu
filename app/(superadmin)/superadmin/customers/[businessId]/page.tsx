...
import { getCustomerDetail, getPlansForPicker } from "@/features/superadmin/services/customer-service";
...
export default async function SuperAdminCustomerDetailPage({
  ...
  const [agent, detail, plans] = await Promise.all([
    findUserByPhone(session.phone),
    getCustomerDetail(businessId),
    getPlansForPicker(),
  ]);
  if (!detail) notFound();
  ...
  return (
    <>
      ...
      <PanelContent>
        <CustomerDetailView detail={detail} plans={plans} />
      </PanelContent>
    </>
  );
}
