...
- `services/`
  ...
  - `otp-service.ts` — abstract `OtpProvider`; `sendOtp()` generates the code, hashes and persists it (`OtpCode`, via `repositories/otp-repository.ts`), then hands it to the active provider. `getOtpProvider()` picks `otp-providers/melipayamak-provider.ts`'s `MelipayamakOtpProvider` when `MELIPAYAMAK_USERNAME`/`MELIPAYAMAK_PASSWORD`/`MELIPAYAMAK_BODY_ID` are set (production), otherwise falls back to `MockOtpProvider` (logs the code instead of sending an SMS — the local dev default). `verifyOtp()` checks the code against the stored hash (rate-limited, single-use, TTL-bound).
