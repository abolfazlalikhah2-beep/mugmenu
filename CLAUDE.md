...
**سایت مارکتینگ (صفحه اصلی، تعرفه‌ها و ...) خارج از این ریپو است** — تصمیم گرفته شد با وردپرس روی دامنه‌ی اصلی ساخته بشه، نه اینجا. این اپ Next.js (auth + منوی عمومی + پنل مدیریت) باید روی یک **ساب‌دامین** جدا دیپلوی بشه (مثلاً `app.مامنو.com` یا مشابه — نام دقیق هنوز مشخص نیست). یعنی:
- مسیر هر رستوران همچنان path-based زیر همون ساب‌دامین می‌مونه (`app.example.com/demo`)، نه ساب‌دامین جدا به‌ازای هر رستوران.
- کوکی session (`magmenu_session`) روی دامنه‌ی پیش‌فرض (host-only) ست می‌شه که برای یک ساب‌دامین مستقل مشکلی نداره؛ فقط اگر بعداً خواستیم بین ساب‌دامین‌ها به اشتراک بذاریمش باید `domain` رو صریح تنظیم کنیم.
- هیچ صفحه‌ی مارکتینگ (Home/Pricing) توی `app/` این پروژه ساخته نمی‌شه.
...
/app                                         → فقط routing و صفحات (thin). منطق تجاری اینجا ممنوعه.
  page.tsx                                   → ریدایرکت به /style-guide (تا وقتی صفحه‌ی اصلی واقعی اضافه بشه)
  ...
  (سایت مارکتینگ اینجا ساخته نمی‌شه — وردپرس روی دامنه‌ی اصلی، این اپ روی ساب‌دامین)
